...
class Sunshine(object):
    ...
    def register_api(self,view, endpoint, url, pk='key'):
        if not url.endswith("/"):
            url = "%s/"%url

        view_func = view.as_view(endpoint)
        self.app.add_url_rule(url, view_func=view_func, defaults={pk: None}, methods=['GET',])
        self.app.add_url_rule(url, view_func=view_func, methods=['POST',])
        self.app.add_url_rule('%s<%s>' % (url, pk), view_func=view_func,
